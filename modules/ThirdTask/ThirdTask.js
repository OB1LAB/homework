...
function calculateAngle(vertex0, vertex1, vertex2) {
  ...
  const thetaDeg = thetaRad.mul(180).div(Decimal.acos(-1)); // 180 / π
  return thetaDeg.toNumber();
}


vertexes.forEach((vertex, vertexIndex) => {
  vertexes[vertexIndex] = rotateVertex(vertex, 0)
})
...
const ThirdTask = () => {
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      const vertex3X = allX.find((item) => allX.filter((a) => a.toNumber() === item.toNumber()).length === 1)
      const vertex3Y = allY.find((item) => allY.filter((a) => a.toNumber() === item.toNumber()).length === 1)
